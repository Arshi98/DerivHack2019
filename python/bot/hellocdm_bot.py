...
def main():
  ...
  async def onInit(event: InitEvent):
    print("Ready & listening for new `UC2` contracts ... ")

  @clientOwner.ledger_created("Main.UC2")  # note: this needs to have a '.' and not a ':'
  async def onCreate(event: ContractCreateEvent):
    allContracts = clientOwner.find(template = "Main.UC2")
