...
def validateCDMJson(cdmDict, className, rosettaUsername, rosettaAuthToken):
  ...
  return requests.post(
    f"https://ui.rosetta-technology.io/api/promoted/validation/{rosettaUsername}/Brillio/latest/{className}",
...
def checkExistingContract(contractName, signatoryName, executionKey, httpEndpointPrefix):
  ...
  if response.status_code == 200:
    result = response.json()["result"]
    for row in result:
      httpExerciseResponse = exerciseChoice(owner, row['contractId'], "ArchiveDuplicates", {}, endpoint)
      print("Archived the existing contract: {}".format(row['contractId']))
...
def buildTradeDetails(cdmData):
  ...
  tradeType = cdmData['primitive']['allocation'][0]['after']['originalTrade']['execution']['executionType']
  tradeAmount = cdmData['primitive']['allocation'][0]['after']['originalTrade']['execution']['settlementTerms']['settlementAmount']['amount']
  currency = cdmData['primitive']['allocation'][0]['after']['originalTrade']['execution']['price']['grossPrice']['currency']['value']
  executionKey = cdmData['primitive']['allocation'][0]['after']['originalTrade']['execution']['meta']['globalKey']
  allocation_1 = cdmData['primitive']['allocation'][0]['after']['allocatedTrade'][0]['execution']['settlementTerms']['settlementAmount']['amount']
  allocation_2 = cdmData['primitive']['allocation'][0]['after']['allocatedTrade'][1]['execution']['settlementTerms']['settlementAmount']['amount']
  ...
  tradeDetails['allocation_1'] = allocation_1
  tradeDetails['allocation_2'] = allocation_2
  print(tradeDetails)
  if not client:
...
if __name__ == '__main__' :
  ...
  if (args.rosetta_user and args.rosetta_token):
    ...
    if httpValidateResponse.status_code != 200 or len(httpValidateResponse.json()) != 0:
      ...
      print(f"#### CDM JSON successfully validated for {cdmEventFileName} ####")
  else:
  ...
  # if tradeDetailsData['tradeAmount'] != tradeDetailsData['allocation_1'] + tradeDetailsData['allocation_2']:
  #   raise Exception(f'Trade allocation is not matcing with trade amount !')
  ...
  checkExistingContract("UC2", owner, tradeDetailsData['executionKey'], endpoint)
